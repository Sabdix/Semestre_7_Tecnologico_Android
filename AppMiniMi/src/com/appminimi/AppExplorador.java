...
public final class AppExplorador extends ListActivity {
	public static final String ACTION_WIDGET = "com.appminimi.manager.AppExplorador.ACTION_WIDGET";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.appexplorador);
        ...
        if (savedInstanceState != null)
        	mHandler = new EventHandler(AppExplorador.this, mFileMag, savedInstanceState.getString("location"));
        else
        	mHandler = new EventHandler(AppExplorador.this, mFileMag);
        ...
        mPathLabel.setText("ruta: /sdcard");
    	...
    	if(multiSelect) {
	    	...
	    	/*zip file */
	    		...
	    		if(mReturnIntent) {
		    		...
		    		CharSequence[] option = {"Extraer aqui", "Extraer a..."};
		    		
		    		builder.setTitle("Extraer");
		    		builder.setItems(option, new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							switch(which) {
								...
								case 1:
									mDetailLabel.setText("Tengo " + item + 
														 " a extraer");
									mHoldingZip = true;
									break;
							}
	    		...
	    		if(file.exists()) {
	    			if(mReturnIntent) {
			    		...
			    		try {
			    			...
			    			Toast.makeText(this, "Lo siento no hay lectores PDF", 
									Toast.LENGTH_SHORT).show();
			    		}
		    		}
	    		}
	    		...
	    		if(file.exists()) {
	    			if(mReturnIntent) {
		    			...
		    			try {
		    				...
		    				Toast.makeText(this, "Lo siento no hay lector HTML", 
		    									Toast.LENGTH_SHORT).show();
		    			}
	    			}
	    		}
	    		...
	    		if(file.exists()) {
	    			if(mReturnIntent) {
			    		...
			    		try {
			    			...
			    			Toast.makeText(this, "Lo siento no se encontro nada " +
			    						   "que abrir. " + file.getName(), 
			    						   Toast.LENGTH_SHORT).show();
			    		}
	    			}
	    		}
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
    	menu.add(0, MENU_MKDIR, 0, "Nueva Carpeta").setIcon(R.drawable.newfolder);
    	menu.add(0, MENU_SEARCH, 0, "Buscar").setIcon(R.drawable.search);
    	
    		/* free space will be implemented at a later time */
//    	menu.add(0, MENU_SPACE, 0, "Free space").setIcon(R.drawable.space);
    	menu.add(0, MENU_SETTING, 0, "Configuraciones").setIcon(R.drawable.setting);
    	menu.add(0, MENU_QUIT, 0, "Cerrar").setIcon(R.drawable.logout);
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo info) {
    	...
    	if(mFileMag.isDirectory(mSelectedListItem) && !mHandler.isMultiSelected()) {
    		menu.setHeaderTitle("Opciones para Carpeta");
        	menu.add(0, D_MENU_DELETE, 0, "Borrar Carpeta");
        	menu.add(0, D_MENU_RENAME, 0, "Renombrar Carpeta");
        	menu.add(0, D_MENU_COPY, 0, "Copiar Carpeta");
        	menu.add(0, D_MENU_MOVE, 0, "Mover(Cortar) Folder");
        	menu.add(0, D_MENU_ZIP, 0, "Comprimir");
        	menu.add(0, D_MENU_PASTE, 0, "Pegar en Carpeta").setEnabled(mHoldingFile || 
        																 multi_data);
        	menu.add(0, D_MENU_UNZIP, 0, "Extraer Aqui").setEnabled(mHoldingZip);
    		
        /* is it a file and is multi-select turned off */
    	} else if(!mFileMag.isDirectory(mSelectedListItem) && !mHandler.isMultiSelected()) {
        	menu.setHeaderTitle("Opciones de Archivo");
    		menu.add(0, F_MENU_DELETE, 0, "Eliminar Archivo");
    		menu.add(0, F_MENU_RENAME, 0, "Renombrar Archivo");
    		menu.add(0, F_MENU_COPY, 0, "Copiar Archivo");
    		menu.add(0, F_MENU_MOVE, 0, "Mover(Cortar) Archivo");
    		menu.add(0, F_MENU_ATTACH, 0, "Enviar");
    	}	
    }
    ...
    public boolean onContextItemSelected(MenuItem item) {
    	...
    	switch(item.getItemId()) {
    		...
    		case F_MENU_DELETE:
    			...
    			builder.setMessage("Eliminar " + mSelectedListItem +
    							" no puede deshacerse. Estas seguro?");
    		...
    		case D_MENU_COPY:
    			...
    			mDetailLabel.setText("Tengo " + mSelectedListItem);
    			return true;
    		...
    		case D_MENU_UNZIP:
    			if(mHoldingZip && mZippedTarget.length() > 1) {
    				...
    				if(new File(mZippedTarget).canRead() && new File(current_dir).canWrite()) {
    					...
    					Toast.makeText(this, "Sin permiso para descomprimir. " + name, 
    							Toast.LENGTH_SHORT).show();
    				}
    			}
    ...
    protected Dialog onCreateDialog(int id) {
    	final Dialog dialog = new Dialog(AppExplorador.this);
    	
    	switch(id) {
    		case MENU_MKDIR:
    			dialog.setContentView(R.layout.input_layout);
    			dialog.setTitle("Crear Nuevo Directorio");
    				...
    				public void onClick (View v) {
    					if (input.getText().length() > 1) {
    						if (mFileMag.createDir(mFileMag.getCurrentDir() + "/", input.getText().toString()) == 0)
    							Toast.makeText(AppExplorador.this, 
    										   "Carpeta " + input.getText().toString() + " creada", 
    										   Toast.LENGTH_LONG).show();
    						else
    							Toast.makeText(AppExplorador.this, "La carpeta no fue creada.", Toast.LENGTH_SHORT).show();
    					}
    		...
    		case F_MENU_RENAME:
    			dialog.setContentView(R.layout.input_layout);
    			dialog.setTitle("Renombrar " + mSelectedListItem);
    			dialog.setCancelable(false);
    			...
    			rename_create.setText("Renombrar");
    			...
    			rename_create.setOnClickListener(new OnClickListener() {
    				public void onClick (View v) {
    					...
    					if(mFileMag.renameTarget(mFileMag.getCurrentDir() +"/"+ mSelectedListItem, rename_input.getText().toString()) == 0) {
    						Toast.makeText(AppExplorador.this, mSelectedListItem + " se renombro a " +rename_input.getText().toString(),
    								Toast.LENGTH_LONG).show();
    					}else
    						Toast.makeText(AppExplorador.this, mSelectedListItem + " no fue renombrada", Toast.LENGTH_LONG).show();
    		...
    		case MENU_SEARCH:
    			...
    			search_label.setText("Buscar un archivo");
   ...
   public boolean onKeyDown(int keycode, KeyEvent event) {
    	...
    	if(keycode == KeyEvent.KEYCODE_SEARCH) {
    		...
    		if(mHandler.isMultiSelected()) {
    			...
    			Toast.makeText(AppExplorador.this, "Multi-seleccion desactivada", Toast.LENGTH_SHORT).show();
    		...
    		Toast.makeText(AppExplorador.this, "Presiona atrás para saliir.", Toast.LENGTH_SHORT).show();
    		
    		if(mHandler.isMultiSelected()) {
    			mTable.killMultiSelect(true);
    			Toast.makeText(AppExplorador.this, "Multi-selección esta desactivada.", Toast.LENGTH_SHORT).show();
    		}
