...
public class EventHandler implements OnClickListener {
	...
	public void onClick(View v) {
		...
		switch(v.getId()) {
			...
			case R.id.back_button:			
				if (mFileMang.getCurrentDir() != "/") {
					if(multi_select_flag) {
						...
						Toast.makeText(mContext, "Multi-seleccion desactivada", 
			...
			case R.id.hidden_copy:
				...
				mInfoLabel.setText("Tengo " + mMultiSelectData.size() + 
								   " archivo(s)");
			...
			case R.id.hidden_delete:	
				...
				builder.setMessage("Estas seguro quieres eliminar " +
								    data.length + " archivos? No podrá " +
								    "deshacerse.");
	...
	private void display_dialog(int type) {
    	...
    	switch(type) {
    		case MANAGE_DIALOG:
    	    	...
    	    	CharSequence[] options = {"Info de Procesos", /*"Wifi Info",*/ "Copia de Seguridad"};
    ...
    public class TableRow extends ArrayAdapter<String> {
    	...
    	public View getView(int position, View convertView, ViewGroup parent) {
    		...
    		if(file.isFile()) {
        		...
        		if(file.isHidden())
        			mViewHolder.bottomView.setText("(ocultos) | " + display_size +" | "+ permission);
        		else
        			mViewHolder.bottomView.setText(display_size +" | "+ permission);
        		
    		} else {
    			if(file.isHidden())
    				mViewHolder.bottomView.setText("(oculto) | " + num_items + " elementos | " + permission);
    			else
    				mViewHolder.bottomView.setText(num_items + " elementos | " + permission);
    		}
    ...
    private class BackgroundWork extends AsyncTask<String, Void, ArrayList<String>> {
    	...
    	protected void onPreExecute() {
    		...
    		switch(type) {
    			case SEARCH_TYPE:
    				pr_dialog = ProgressDialog.show(mContext, "Buscando", 
    												"Buscando en sistema...",
    												true, true);
    				break;
    				
    			case COPY_TYPE:
    				pr_dialog = ProgressDialog.show(mContext, "Copiando", 
    												"Copiando archivos...", 
    												true, false);
    				break;
    				
    			case UNZIP_TYPE:
    				pr_dialog = ProgressDialog.show(mContext, "Descomprimir", 
    												"Descomprimiendo espera...",
    												true, false);
    				break;
    				
    			case UNZIPTO_TYPE:
    				pr_dialog = ProgressDialog.show(mContext, "Descomprimiendo", 
    			...
    			case DELETE_TYPE:
    				pr_dialog = ProgressDialog.show(mContext, "Borrando", 
    												"Borrando Archivos...", 
    												true, false);
    				break;
    		}
		...
		protected void onPostExecute(final ArrayList<String> file) {			
			...
			switch(type) {
				case SEARCH_TYPE:				
					if(len == 0) {
						Toast.makeText(mContext, "No se encontro " + file_name, 
											Toast.LENGTH_SHORT).show();
						...
						AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
						builder.setTitle("Encontrados " + len + " archivo(s)");
				...
				case COPY_TYPE:
					...
					if(copy_rtn == 0)
						Toast.makeText(mContext, "Archivo copiado y pegado con exito", 
											Toast.LENGTH_SHORT).show();
					else
						Toast.makeText(mContext, "Copiar pegar fallo", Toast.LENGTH_SHORT).show();
